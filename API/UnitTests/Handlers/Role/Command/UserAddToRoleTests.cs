...
{
    public class UserAddToRoleTests
    {
        private readonly Mock<UserManager<Common.Entities.User>> _mockUserManager;
        private readonly Mock<RoleManager<IdentityRole>> _mockRoleManager;
        private readonly UserAddToRoleHandler _handler;

        public UserAddToRoleTests()
        {
            ...
            _handler = new UserAddToRoleHandler(_mockUserManager.Object, _mockRoleManager.Object);
        }
        ...
        public async Task Handle_WhenValidationFails_ShouldThrowValidationException()
        {
            ...
            var command = new UserAddToRoleCommand { UserId = "1", RoleId = "2" };
        ...
        [Fact]
        public async Task Handle_WhenUserAlreadyHasRole_ShouldThrowValidationException()
        {
            ...
            var exception = await Assert.ThrowsAsync<ValidationException>(() => _handler.Handle(command, CancellationToken.None));
            Assert.Contains("Bu rol istifadecide movcuddur", exception.Errors);
        }

        [Fact]
        public async Task Handle_WhenRoleAssignedSuccessfully_ShouldReturnSuccessResponse()
        {
            ...
            _mockUserManager.Setup(x => x.AddToRoleAsync(user, role.Name)).ReturnsAsync(IdentityResult.Success);

           
            var result = await _handler.Handle(command, CancellationToken.None);

            
            _mockUserManager.Verify(x => x.AddToRoleAsync(user, role.Name), Times.Once);
            Assert.Equal("Istifadeciye rol elave olundu", result.Message);
        }
    }
}
